...
def s1():
    ...
    if os.path.exists("save3.txt"):
        os.remove("save3.txt")
    if os.path.exists("save4.txt"):
        os.remove("save4.txt")
    if os.path.exists("save5.txt"):
        os.remove("save5.txt")
    while True:
        PLAY_MOUSE_POS = pygame.mouse.get_pos()
        SCREEN.fill("black")
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if VYB_BUTTONP.checkForInput(PLAY_MOUSE_POS):
                    pesek()
                if VYB_BUTTON1.checkForInput(PLAY_MOUSE_POS):
                    pesek()
                if VYB_BUTTON2.checkForInput(PLAY_MOUSE_POS):
                    pesek()
                if VYB_BUTTON3.checkForInput(PLAY_MOUSE_POS):
                    pesek()
                if VYB_BUTTON4.checkForInput(PLAY_MOUSE_POS):
                    pesek()
            pygame.display.update()
def pesek():
...
def sdum232():
    while True:
        ...
        imp2 = pygame.image.load('postavy/cednik.png').convert()                                                          
        SCREEN.blit(imp2, ((Width/100)*32,(Height/100)*18.518)) 
        VIK1_BUTTON.changeColor(PLAY_MOUSE_POS)
        VIK2_BUTTON.changeColor(PLAY_MOUSE_POS)
        VIK1_BUTTON.update(SCREEN)
        VIK2_BUTTON.update(SCREEN)
        DALSI_BUTTON.update(SCREEN)
        PLAY_BACK.changeColor(PLAY_MOUSE_POS)
        PLAY_BACK.update(SCREEN)
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if PLAY_BACK.checkForInput(PLAY_MOUSE_POS):
                    POMOC()
                if VIK1_BUTTON.checkForInput(PLAY_MOUSE_POS):
                    sdum24()    
                if VIK2_BUTTON.checkForInput(PLAY_MOUSE_POS):
                    sdum26spatne()     
...
def sls5():
    ...
    if os.path.exists("save4.txt"):
        os.remove("save4.txt")
    while True:
        PLAY_MOUSE_POS = pygame.mouse.get_pos();SCREEN.fill("black");SCREEN.blit(resized_ls, (0, 0));f = open("save5.txt", "w");f.close()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit();sys.exit()
            s222n()
...
def POMOC():
    ...
    save5=os.path.exists("save5.txt")
